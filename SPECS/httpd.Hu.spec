...
#+Hu3 MPMs (Multi-Processing Module)
Patch70:		httpd-2.2.3-peruser-0.3.0.patch
Patch71:		apache2.2-mpm-itk-20080727-00_PatchOnPatch_forMergeWithPeruser.patch
Patch72:		http://mpm-itk.sesse.net/apache2.2-mpm-itk-20080727-00.patch
...
Hubbitus: this build include 2 additional MPM:
	Peruser ( http://www.telana.com/peruser.php )
	 and
	Itk (http://mpm-itk.sesse.net/)
...
#+Hu
%patch70 -p1 -b .peruser

#+Hu
patch72filename=$( basename %{PATCH72} )
cp %{PATCH72} "$patch72filename"
#Redefine to patched patch!
%patch71 -p0 -b .itk-peruser

#+Hu Can't use satndart macros
#%patch72 -p1 -b .itk
%__patch -p1 -b --suffix .itk -s < $patch72filename
...
#+Hu peruser
mpmbuild peruser --enable-modules=none
...
#+Hu+2
install -m 755 peruser/httpd $RPM_BUILD_ROOT%{_sbindir}/httpd.peruser
...
#+Hu Verify that the same modules were built into the httpd binaries
#+Hu As this done fore worker above.
	for mpm in peruser; do
	./${mpm}/httpd -l | grep -v ${mpm} > ${mpm}.mods
		if ! diff -u prefork.mods ${mpm}.mods; then
		: Different modules built into httpd binaries, will not proceed
		exit 1
...
%changelog
* Sun Nov 9 2008 Pavel Alexeev <Pahan [ at ] Hubbitus [ DOT ] spb [ dOt.] su> - 2.2.9-1.Hu.0
- New version 2.2.9
- Replace Patch3 httpd-2.0.45-deplibs.patch to httpd-2.2.9-deplibs.patch from upstream Fedora httpd-2.2.9-1.fc9.src.rpm
- Update Patch72: http://mpm-itk.sesse.net/apache2.2-mpm-itk-20080105-00.patch to http://mpm-itk.sesse.net/apache2.2-mpm-itk-20080727-00.patch
- Accordinglu update: Patch71: apache2.2-mpm-itk-20080105-00_PatchOnPatch_forMergeWithPeruser.patch to apache2.2-mpm-itk-20080727-00_PatchOnPatch_forMergeWithPeruser.patch

* Mon Apr 21 2008 Pavel Alexeev <Pahan [ at ] Hubbitus [ DOT ] spb [ dOt.] su> - 2.2.8-0.Hu.0
- New version 2.2.8
- Reformat with tabs.
- Update patch72 to http://mpm-itk.sesse.net/apache2.2-mpm-itk-20080105-00.patch
	and accordingly Patch71 adjusted and renamed to: apache2.2-mpm-itk-20080105-00_PatchOnPatch_forMergeWithPeruser.patch
- Remove Patch56:		httpd-2.2.6-ssllibver.patch, which is not compatible with new version.

* Tue Dec 25 2007 Pavel Alexeev <Pahan [ at ] Hubbitus [ DOT ] info> 2.2.6-3
- Import from http://download.fedora.redhat.com/pub/fedora/linux/releases/8/Fedora/source/SRPMS/httpd-2.2.6-3.src.rpm
- Add Peruser patch Patch70: (http://www.telana.com/files/httpd-2.2.3-peruser-0.3.0.patch)
- Add (selfmade) Patch71: apache2.2-mpm-itk-20070425-00_PatchOnPatch_forMergeWithPeruser.patch Patch to merge 2 patches: itk and peruser!
- Add MPM-ITK patch Patch72: (http://mpm-itk.sesse.net/apache2.2-mpm-itk-20070425-00.patch)
- Add info about additional MPMs into description of package.
- Change mod_ssl package name to httpd-mod_ssl (remove -n parameter in %package, %description, %files, %post)
	- Add for it package: Provides: mod_ssl, Obsoletes: mod_ssl
