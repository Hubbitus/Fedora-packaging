...
if test $1 -eq 1
then
	/usr/sbin/chkfontpath --add %{fontdir} &>/dev/null || :
fi
...
if test $1 -eq 0
then
	/usr/sbin/chkfontpath --remove %{fontdir}  &>/dev/null || :
fi
...
%changelog
* Fri Jul 15 2011 Pavel Alexeev <Pahan@Hubbitus.info> - 2.0-2
- Disable requirement chkfontpath
